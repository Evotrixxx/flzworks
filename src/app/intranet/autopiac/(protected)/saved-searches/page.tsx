...
import { AUTOPIAC_BASE_PATH, autopiacPath } from "@/lib/routes";
...
function savedSearchHref(query: string, locale: string) {
  ...
  return `${AUTOPIAC_BASE_PATH}?${params.toString()}`;
}
...
export default async function SavedSearchesPage({ searchParams }: { searchParams: Promise<SearchParamsInput> }) {
  ...
  if (!user) {
    redirect(`/login?redirect=${encodeURIComponent(autopiacPath("/saved-searches"))}&lang=${locale}`);
  }
